// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {main} from '../models';

export function AppendHistory(arg1:main.HistoryEntry):Promise<void>;

export function ClearHistory():Promise<void>;

export function DeleteCollection(arg1:string):Promise<void>;

export function DeleteRequest(arg1:string,arg2:string):Promise<void>;

export function Export(arg1:main.Request,arg2:Array<any>,arg3:Array<string>,arg4:main.RequestResult):Promise<void>;

export function ListCollections():Promise<Array<main.Collection>>;

export function ListHistory(arg1:number):Promise<Array<main.HistoryEntry>>;

export function LoadSession():Promise<main.Session>;

export function MakeRequest(arg1:string,arg2:string,arg3:string,arg4:main.Headers):Promise<main.RequestResult>;

export function RunCurl(arg1:string):Promise<main.RequestResult>;

export function SaveCollection(arg1:main.Collection):Promise<void>;

export function SaveRequest(arg1:string,arg2:main.SavedRequest):Promise<void>;

export function SaveSession(arg1:main.Session):Promise<void>;

export function SaveTextFile(arg1:string,arg2:string):Promise<void>;

export function Send(arg1:main.RequestSpec):Promise<main.RequestResult>;
